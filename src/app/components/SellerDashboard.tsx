...
export function SellerDashboard() {
  ...
  const acceptMeetup = async (meetup: Meetup) => {
    const { data, error } = await supabase.from("meetup_proposals").update({ seller_accepted: true, status: meetup.buyer_accepted ? "confirmed" : "pending", updated_at: new Date().toISOString() }).eq("order_id", meetup.order_id).select().single();
    if (error) { alert(error.message); return; }
    ...
    <div className="min-h-screen bg-slate-50 py-8">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        <div>
          ...
          <p className="text-gray-600 mt-1">Manage your listings and buyer requests.</p>
          {newOrderNotice && (
            <div className="mt-4 flex items-center justify-between rounded-lg border border-orange-200 bg-orange-50 px-4 py-3 text-sm text-orange-900">
              <span>A new buyer order request has arrived.</span>
              <Button size="sm" variant="outline" onClick={() => setNewOrderNotice(false)}>Dismiss</Button>
            </div>
          )}
        </div>
        <Card><CardHeader><CardTitle className="flex items-center gap-2"><Clock3 className="h-5 w-5 text-blue-600" /> Buyer Requests</CardTitle></CardHeader><CardContent className="space-y-3">
          ...
          {orders.map((order) => <div key={order.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border rounded-lg p-4 bg-white"><div><p className="font-semibold">{order.product?.title || "Product"}</p><p className="text-sm text-gray-600">Requested by {order.buyer?.full_name || "Student"} · RM {Number(order.price).toFixed(2)}{order.product?.product_type === "clubmerch" && <> · Quantity: {order.quantity ?? 1}</>}</p><p className="text-xs text-gray-400 mt-1">{new Date(order.created_at).toLocaleString()}</p></div><div className="flex items-center gap-2"><Badge className={order.status === "accepted" ? "bg-green-100 text-green-800" : order.status === "rejected" || order.status === "cancelled" ? "bg-red-100 text-red-800" : "bg-yellow-100 text-yellow-800"}>{order.status}</Badge>{order.status === "pending" && <><Button size="sm" onClick={() => updateOrder(order, "accepted")} className="bg-green-600 hover:bg-green-700"><Check className="h-4 w-4 mr-1" /> Accept</Button><Button size="sm" variant="outline" onClick={() => updateOrder(order, "rejected")}><X className="h-4 w-4 mr-1" /> Reject</Button></>}{order.status === "accepted" && <Button size="sm" variant="outline" onClick={() => updateOrder(order, "completed")}>Mark completed</Button>}<Button size="sm" variant="ghost" onClick={() => navigate(`/chat/${order.buyer_id}?orderId=${order.id}`)}>Message</Button></div></div>)}
        </CardContent></Card>
